...
const store = createStore({
  ...
  actions: {
    logout({ commit }) {
      return axios
        .get('https://json-server-swart-rho.vercel.app/agregator_users')
        .then(() => {
    ...
    login({ commit }, credentials) {
      let user = store.state.users.find(
      ...
      if (user.password == credentials.password && user.role == 'user') {
        return axios
          .get(
            'https://json-server-swart-rho.vercel.app/agregator_users',
            credentials
          )
          .then((response) => {
    ...
    addMovie({ commit }, movie) {
      return axios
        .post(
          'https://json-server-swart-rho.vercel.app/agregator_movies',
          movie
        )
        .then((response) => {
    ...
    editMovie({ commit }, movie) {
      return axios
        .put(
          `https://json-server-swart-rho.vercel.app/agregator_movies/${movie.id}`,
          {
            title: movie.title,
            poster: movie.poster,
    ...
    deleteMovie({ commit }, id) {
      return axios
        .delete(
          `https://json-server-swart-rho.vercel.app/agregator_movies/${id}`
        )
        .then(() => {
          commit(
            'SET_MOVIES',
            state.movies.filter((m) => m.id !== id)
          );
        })
        .catch((error) => {
          console.error(error);
